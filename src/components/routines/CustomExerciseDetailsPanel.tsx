...
const CustomExerciseDetailsPanel = ({ exercise }: Props) => {
    if (!exercise) {
        return (
            <div className="flex flex-col gap-6 w-full md:sticky md:top-24 md:mt-0 lg:-mt-2 h-[400px] border rounded-xl items-center justify-center p-6 text-center shadow-sm bg-card">
    ...
    return (
        <div className="flex flex-col gap-6 w-full md:sticky md:top-24 md:mt-0 lg:-mt-2 bg-card border rounded-xl shadow-sm p-6 overflow-hidden max-h-[calc(100vh-8rem)] overflow-y-auto">
            <h2 className="text-2xl font-bold tracking-tight capitalize border-b pb-4">{exercise.name}</h2>

            <div className="flex justify-center bg-white rounded-lg p-2 overflow-hidden border">
                <img
                    src={exercise.gifUrl}
                    alt={exercise.name}
                    className="w-full max-w-[300px] rounded object-cover"
                />
            </div>
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {exercise.targetMuscles.map(m => (
                            <span key={`target-${m}`} className="px-2 py-1 bg-primary/10 text-primary rounded text-xs font-medium capitalize border border-primary/20">
                                {m}
                            </span>
                        ))}
                        {exercise.secondaryMuscles.map(m => (
                            <span key={`sec-${m}`} className="px-2 py-1 bg-secondary text-secondary-foreground rounded text-xs font-medium capitalize border border-secondary">
                                {m}
                            </span>
                        ))}
                    </div>
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {exercise.equipments.map(e => (
                            <span key={`eq-${e}`} className="px-2 py-1 bg-accent text-accent-foreground rounded text-xs font-medium capitalize">
                                {e}
                            </span>
                        ))}
                    </div>
